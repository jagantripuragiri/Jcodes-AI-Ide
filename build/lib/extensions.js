...
function doPackageLocalExtensionsStream(forWeb, disableMangle, native) {
    ...
    if (forWeb) {
        ...
        const dependenciesSrc = productionDependencies.map(d => path_1.default.relative(root, d)).filter(d => d).map(d => [`${d}/**`, `!${d}/**/{test,tests}/**`]).flat();
        if (!dependenciesSrc.length) dependenciesSrc.push('dummy_non_existent_folder_xyz/**');
