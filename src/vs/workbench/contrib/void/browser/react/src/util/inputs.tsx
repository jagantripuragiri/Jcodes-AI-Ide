...
import { addImageFilesToStaging } from './imageAttachments.js';
...
export const VoidInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, enableImageAttach, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	return <>
		<textarea
			...
			onPaste={useCallback((e: React.ClipboardEvent<HTMLTextAreaElement>) => {
				if (!enableImageAttach) return
				const imageFiles = Array.from(e.clipboardData.items)
					.filter(item => item.kind === 'file' && item.type.startsWith('image/'))
					.map(item => item.getAsFile())
					.filter((f): f is File => f !== null)
				if (!imageFiles.length) return // no images in clipboard - let normal text paste proceed
				e.preventDefault()
				addImageFilesToStaging(chatThreadService, imageFiles)
			}, [enableImageAttach, chatThreadService])}
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-vscode-list-active-bg text-vscode-list-active-fg' : 'bg-void-bg-2-alt text-void-fg-1'}
										`}
						...
						{options.map((option) => {
							...
							return (
								<div
									...
									${thisOptionIsSelected ? 'bg-vscode-list-active-bg text-vscode-list-active-fg' : 'hover:bg-vscode-list-active-bg hover:text-vscode-list-active-fg'}
								`}
									onClick={() => {
