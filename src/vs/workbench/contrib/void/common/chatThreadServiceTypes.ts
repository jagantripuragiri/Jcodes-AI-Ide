...
import { AnthropicReasoning, ImageMimeType, RawToolParamsObj } from './sendLLMMessageTypes.js';
...
export type StagingSelectionItem = {
	...
	type: 'Image';
	uri: URI; // synthetic identity URI (scheme 'void-image'), used only for dedup/keying/tooltip fallback - never read via IFileService
	filename: string;
	mimeType: ImageMimeType;
	base64Data: string; // raw base64, no "data:...;base64," prefix - already resized/downscaled client-side
	width: number;
	height: number;
	sizeBytes: number;
	language?: undefined;
