...
const AddButton = ({ disabled, text = 'Add', ...props }: { disabled?: boolean, text?: React.ReactNode } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	...
	return <button
		disabled={disabled}
		className={`bg-black text-white dark:bg-white dark:text-black px-3 py-1 rounded-sm ${!disabled ? 'hover:bg-black/80 dark:hover:bg-white/80 cursor-pointer' : 'opacity-50 cursor-not-allowed bg-opacity-70'}`}
		{...props}
	>{text}</button>
		...
		>
			...
			>
				...
				<div className="flex justify-end gap-2 mt-4">
					...
					<VoidButtonBgDarken
						onClick={onSave}
						className="px-3 py-1 bg-black text-white dark:bg-white dark:text-black"
					>
						Save
					</VoidButtonBgDarken>
				</div>
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	return <div className=''>
		...
		{showCheckmark ? (
			<div className="mt-4">
				<AnimatedCheckmarkButton text='Added' className="bg-black text-white dark:bg-white dark:text-black px-3 py-1 rounded-sm" />
			</div>
		) : isAddModelOpen ? (
			<div className="mt-4">
			...
			<div className='relative'>
				...
				{isPasswordField && (
					<button
						...
						className='absolute right-2 top-1/2 -translate-y-1/2 text-void-fg-3 hover:text-void-fg-1'
						data-tooltip-id='void-tooltip'
						data-tooltip-content={isRevealed ? 'Hide' : 'Show'}
					>
...
// shows which model the API key/endpoint on this page is actually being used for
const ActiveModelIndicator = () => {
	const settingsState = useSettingsState()
	const chatModelSelection = settingsState.modelSelectionOfFeature['Chat']

	if (!chatModelSelection) {
		return <WarningBox className="mb-4" text='No model is currently selected. Add a model below to get started.' />
	}

	const { title: providerTitle } = displayInfoOfProviderName(chatModelSelection.providerName)

	return <div className='flex items-center gap-2 mb-4 py-2 px-3 bg-void-bg-2 rounded text-sm'>
		<Check className='text-green-500 shrink-0' size={14} />
		<span className='text-void-fg-2'>
			Currently using <span className='font-medium text-void-fg-1'>{chatModelSelection.modelName}</span> via <span className='font-medium text-void-fg-1'>{providerTitle}</span>
		</span>
	</div>
}
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`J code's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							{/* Feature Options section - hidden, not needed for this build */}
							...
							{/* General section - hidden, not needed for this build */}
							<div className={`hidden flex flex-col gap-12`}>
...
Alternatively, place a \`.jcodesrules\` file in the root of your workspace.
							...
							{/* MCP section - hidden, not needed for this build */}
							<div className={'hidden'}>
								<ErrorBoundary>
