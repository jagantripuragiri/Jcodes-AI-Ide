...
import { ABOUT_ACTIVITY_ID, ACCOUNTS_ACTIVITY_ID, GLOBAL_ACTIVITY_ID } from '../../common/activity.js';
...
// command id of the About action contributed by the void contrib layer (contribs/void/browser/voidAboutPane.ts);
// referenced by string here to avoid a browser/parts -> contrib layering dependency
const VOID_SHOW_ABOUT_COMMAND_ID = 'workbench.action.showVoidAbout';

export class GlobalCompositeBar extends Disposable {

	private static readonly ACCOUNTS_ACTION_INDEX = 0;
	// number of always-visible actions besides the (optional) accounts action
	private static readonly NON_ACCOUNTS_ACTIONS_COUNT = 2; // about + manage
	...
	static readonly ABOUT_ICON = registerIcon('about-view-bar-icon', Codicon.info, localize('aboutViewBarIcon', "About icon in the view bar."));
	...
	private readonly aboutAction = this._register(new Action(ABOUT_ACTIVITY_ID));
	...
	constructor(
		...
		this.globalActivityActionBar = this._register(new ActionBar(this.element, {
			actionViewItemProvider: (action, options) => {
				...
				if (action.id === ABOUT_ACTIVITY_ID) {
					return this.instantiationService.createInstance(AboutActivityActionViewItem, { ...options, colors: this.colors, hoverOptions: this.activityHoverOptions });
				}
		...
		this.globalActivityActionBar.push(this.aboutAction);
	...
	private toggleAccountsActivity() {
		const sizeWithAccounts = GlobalCompositeBar.NON_ACCOUNTS_ACTIONS_COUNT + 1;
		if (this.globalActivityActionBar.length() === sizeWithAccounts && this.accountsVisibilityPreference) {
			return;
		}
		if (this.globalActivityActionBar.length() === sizeWithAccounts) {
...
export class AboutActivityActionViewItem extends CompositeBarActionViewItem {
	...
	constructor(
		...
		const action = instantiationService.createInstance(CompositeBarAction, {
			id: ABOUT_ACTIVITY_ID,
			name: localize('about', "About"),
			classNames: ThemeIcon.asClassNameArray(GlobalCompositeBar.ABOUT_ICON)
		});
	...
	override render(container: HTMLElement): void {
		...
		this._register(addDisposableListener(this.container, EventType.MOUSE_DOWN, (e: MouseEvent) => {
			EventHelper.stop(e, true);
			this.commandService.executeCommand(VOID_SHOW_ABOUT_COMMAND_ID);
		}));
