...
import { LLMUsage } from './tokenUsageTypes.js'
...
export type AnthropicLLMChatMessage = {
	...
	imageParts?: { mimeType: ImageMimeType; base64Data: string; }[]; // transient sidecar, stripped before this array is finalized
}
export type OpenAILLMChatMessage = {
	role: 'system' | 'user' | 'developer';
	content: string | (
		| { type: 'text'; text: string; }
		| { type: 'image_url'; image_url: { url: string; }; } // url is "data:<mime>;base64,<data>"
	)[];
	imageParts?: { mimeType: ImageMimeType; base64Data: string; }[]; // transient sidecar, stripped before this array is finalized
} | {
	role: 'assistant',
...
export type OnFinalMessage = (p: { fullText: string; fullReasoning: string; toolCall?: RawToolCallObj; anthropicReasoning: AnthropicReasoning[] | null; usage: LLMUsage | null }) => void // id is tool_use_id
