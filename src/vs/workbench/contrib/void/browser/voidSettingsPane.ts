...
class VoidSettingsInput extends EditorInput {
	...
	override getName(): string {
		return nls.localize('voidSettingsInputsName', 'J code\'s Settings');
	}
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(VoidSettingsPane, VoidSettingsPane.ID, nls.localize('VoidSettingsPane', "J code\'s Settings Pane")),
	[new SyncDescriptor(VoidSettingsInput)]
);


// J code's Settings is opened from the chat panel's own gear button (see sidebarActions.ts) and
// from the bottom-left Manage gear menu, so it's intentionally not duplicated in the title bar's
// layout controls.
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('voidSettings', "J code's: Toggle Settings"),
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('voidSettingsAction2', "J code's: Open Settings"),
			f1: true,
			icon: Codicon.settingsGear,
		});
	}
...
MenuRegistry.appendMenuItem(MenuId.GlobalActivity, {
	...
	command: {
		id: VOID_TOGGLE_SETTINGS_ACTION_ID,
		title: nls.localize('voidSettingsActionGear', "J code\'s Settings")
	},
	order: 1
});
