...
import { LLMUsage, estimateTokenCount } from '../common/tokenUsageTypes.js';
...
import { THREAD_STORAGE_KEY, THREAD_STORAGE_KEY_FALLBACKS } from '../common/storageKeys.js';
...
// rough text approximation of a prompt, used only to estimate token counts when a provider doesn't report real usage
const flattenLLMMessageText = (messages: LLMChatMessage[]): string => {
	const chunks: string[] = []
	for (const m of messages) {
		const content: unknown = (m as { content?: unknown }).content ?? (m as { parts?: unknown }).parts
		if (typeof content === 'string') { chunks.push(content); continue }
...
export type ThreadType = {
	...
	usage: LLMUsage; // running total of tokens used across this thread (estimated for providers that don't report it)
...
// the shape actually written to storage - excludes fields that only make sense for a live thread
// (mountedInfo's Promise/resolver/DOM ref, and the filesWithUserChanges Set, which doesn't round-trip
// through JSON anyway). See _sanitizeThreadsForStorage.
type SerializableThread = Omit<ThreadType, 'filesWithUserChanges'> & {
	state: Omit<ThreadType['state'], 'mountedInfo'>
}
type SerializableChatThreads = {
	[id: string]: undefined | SerializableThread;
}
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	dangerousSetState = (newState: ThreadsState) => {
		this.state = newState
		this._storeAllThreads(newState.allThreads) // eg importing threads from a backup file - persist immediately, don't wait for the next edit
	...
	private _reviveThreadURIs = (key: string, value: any) => {
	...
	private _convertThreadDataFromStorage(threadsStr: string): ChatThreads {
		return JSON.parse(threadsStr, this._reviveThreadURIs);
	}

	// parses threads one-by-one so a single corrupted thread (eg an invalid URI or message) can't take down the rest of the chat history
	private _rescueThreadsFromCorruptStorage(threadsStr: string): ChatThreads | null {
		let rawThreads: unknown
		try {
			rawThreads = JSON.parse(threadsStr) // plain parse, no URI revival - only fails on real JSON syntax errors
		}
		catch (e) {
			console.error('[Void] Stored chat threads are not valid JSON; chat history for this key is unrecoverable.', e)
			return null
		}

		if (!rawThreads || typeof rawThreads !== 'object') return null

		const rescued: ChatThreads = {}
		let numRescued = 0
		let numDropped = 0
		for (const threadId in rawThreads as Record<string, unknown>) {
			try {
				// re-parse just this thread with URI revival, in isolation, so a bad URI/message in one thread can't drop the rest
				rescued[threadId] = JSON.parse(JSON.stringify((rawThreads as Record<string, unknown>)[threadId]), this._reviveThreadURIs)
				numRescued += 1
			}
			catch (e) {
				console.error(`[Void] Dropping corrupted chat thread "${threadId}" while recovering chat history.`, e)
				numDropped += 1
			}
		}
		console.warn(`[Void] Recovered ${numRescued} chat thread(s)${numDropped > 0 ? `; dropped ${numDropped} corrupted thread(s)` : ''}.`)

		return rescued
	}

	// storage never contains mountedInfo or filesWithUserChanges (stripped by _sanitizeThreadsForStorage
	// before every save - see below), so threads loaded from disk are missing fields ThreadType requires.
	// Refill them here so loaded threads have the exact same shape as a freshly-created thread.
	private _normalizeLoadedThreads(threads: ChatThreads): ChatThreads {
		const normalized: ChatThreads = {}
		for (const threadId in threads) {
			...
			normalized[threadId] = {
				...thread,
				filesWithUserChanges: new Set(),
				usage: thread.usage ?? { inputTokens: 0, outputTokens: 0 }, // older threads saved before usage tracking existed
			}
		}
		return normalized
	}
	...
	private _readAllThreads(): ChatThreads | null {
		...
		if (!threadsStr) {
			for (const fallbackKey of THREAD_STORAGE_KEY_FALLBACKS) {
				threadsStr = this._storageService.get(fallbackKey, StorageScope.APPLICATION);
				if (threadsStr) {
					console.warn(`[Void] No chat threads found under "${THREAD_STORAGE_KEY}"; recovered threads from legacy key "${fallbackKey}".`)
					break
				}
			}
		}
		...
		try {
			return this._normalizeLoadedThreads(this._convertThreadDataFromStorage(threadsStr));
		}
		catch (e) {
			console.error('[Void] Failed to parse stored chat threads; attempting to rescue individual threads instead of discarding all chat history.', e)
			const rescued = this._rescueThreadsFromCorruptStorage(threadsStr)
			return rescued ? this._normalizeLoadedThreads(rescued) : null
		}
	}

	// strips runtime-only state (mountedInfo's Promise/resolver/DOM ref, and the filesWithUserChanges
	// Set, which JSON.stringify would silently turn into `{}` anyway) from each thread before storing.
	// mountedInfo in particular must never reach JSON.stringify: it holds a pending Promise and a
	// resolver closure tied to a React ref for whichever thread is currently mounted in the UI, none of
	// which are meaningful (or safely serializable) across an app restart.
	private _sanitizeThreadsForStorage(threads: ChatThreads): SerializableChatThreads {
		const sanitized: SerializableChatThreads = {}
		for (const threadId in threads) {
			const thread = threads[threadId]
			if (!thread) continue

			const { mountedInfo, ...serializableState } = thread.state
			const { filesWithUserChanges, ...serializableThread } = thread
	...
	private _storeAllThreads(threads: ChatThreads) {
		const sanitizedThreads = this._sanitizeThreadsForStorage(threads)

		let serializedThreads: string
		try {
			serializedThreads = JSON.stringify(sanitizedThreads);
		}
		catch (e) {
			console.error('[Void] Failed to serialize chat threads; skipping this save to avoid corrupting stored chat history.', e)
			return
		}
	...
	private async _runChatAgent({
		...
		while (shouldSendAnotherMessage) {
			...
			while (shouldRetryLLM) {
				...
				type ResTypes =
					| { type: 'llmDone', toolCall?: RawToolCallObj, info: { fullText: string, fullReasoning: string, anthropicReasoning: AnthropicReasoning[] | null, usage: LLMUsage | null } }
				...
				this._recordUsage(threadId, info.usage ?? {
					inputTokens: estimateTokenCount(flattenLLMMessageText(messages)),
					outputTokens: estimateTokenCount(info.fullText + info.fullReasoning),
				}, modelSelection, estimateTokenCount(separateSystemMessage ?? ''))
...
*/
	...
	// first user message's text, used as a stable snapshot of "which conversation was this" for the token usage dashboard
	// (mirrors the title shown in the thread selector - see SidebarThreadSelector.tsx)
	private _deriveThreadTitle(oldThread: ThreadType): string {
		const firstUserMsg = oldThread.messages.find(msg => msg.role === 'user')
		return (firstUserMsg?.role === 'user' && firstUserMsg.displayContent) || 'New Chat'
	}

	// adds to this thread's running token total, and to the app-wide daily total (used for weekly/monthly stats)
	private _recordUsage(threadId: string, usage: LLMUsage, modelSelection: ModelSelection | null, systemTokens: number) {
		const { allThreads } = this.state
		const oldThread = allThreads[threadId]
		if (!oldThread) return // should never happen
		...
		const newThreads = {
			...
			[oldThread.id]: {
				...oldThread,
				usage: {
					inputTokens: oldThread.usage.inputTokens + usage.inputTokens,
					outputTokens: oldThread.usage.outputTokens + usage.outputTokens,
				},
			}
		}
		this._storeAllThreads(newThreads)
		...
		this._tokenUsageService.recordUsage(usage, {
			systemTokens,
			model: modelSelection?.modelName ?? 'unknown',
			providerName: modelSelection?.providerName ?? 'unknown',
			threadId,
			threadTitle: this._deriveThreadTitle(oldThread),
		})
	}
