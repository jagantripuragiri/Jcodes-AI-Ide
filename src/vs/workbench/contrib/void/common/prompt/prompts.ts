...
export const chat_systemMessage = ({ workspaceFolders, openedURIs, activeURI, persistentTerminalIDs, directoryStr, chatMode: mode, mcpTools, includeXMLToolDefinitions, projectBrainContext }: { workspaceFolders: string[], directoryStr: string, openedURIs: string[], activeURI: string | undefined, persistentTerminalIDs: string[], chatMode: ChatMode, mcpTools: InternalToolInfo[] | undefined, includeXMLToolDefinitions: boolean, projectBrainContext?: string | null }) => {
	const header = (`You are J code's, an expert AI coding ${mode === 'agent' ? 'agent' : 'assistant'} whose job is \
...
${directoryStr}
	...
	const brainInfo = projectBrainContext ? (`Here is a summary from Project Brain, J code's persistent understanding of this repository. \
Use it to stay consistent with the project's existing architecture and conventions, but still verify specifics with tools before making changes - this summary can be incomplete or out of date:
<project_brain_context>
${projectBrainContext}
</project_brain_context>`) : null


	const userBio = (`Here is background info about the developer of this app, in case the user asks things like "who developed this app", "who made this", "who is Jagan Tripuragiri", or similar:
<developer_bio>
This app ("J code's") was developed by Jagan Tripuragiri, a developer, AI engineer, and technology enthusiast from Andhra Pradesh, India, focused on building practical software, AI-powered applications, automation systems, and digital products. He is a Computer Science/IT graduate of Maharaj Vijayaram Gajapathi Raj College of Engineering (MVGR) and has built projects across web development, AI, and automation.

He is associated with Open Seva, an initiative helping small and micro businesses access digital technology (simple websites, AI chat assistants, branding, and technical support) for businesses without large tech budgets.

He learns primarily by building rather than pure theory, frequently experimenting with new technologies to turn them into useful products or businesses. His interests extend into AI entrepreneurship, SaaS products, automation, and startups. His public LinkedIn profile describes him as a computer science student passionate about technology and algorithms, and lists experience associated with Reaidy.io.

Only bring this up if the user asks about the app's developer/creator or about Jagan Tripuragiri specifically; do not volunteer it otherwise.
</developer_bio>`)
...
${details.map((d, i) => `${i + 1}. ${d}`).join('\n\n')}`)
	...
	ansStrs.push(fsInfo)
	if (brainInfo) ansStrs.push(brainInfo)
	ansStrs.push(userBio)
...
export const messageOfSelection = async (
	...
	if (s.type === 'CodeSelection') {
		...
		// images are sent as separate provider content blocks (see imageParts sidecar in convertToLLMMessageService.ts), not flattened into text
		return ''
	}
	else
		return ''
