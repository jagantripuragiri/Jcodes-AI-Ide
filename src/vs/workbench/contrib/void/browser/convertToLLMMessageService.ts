...
import { AnthropicLLMChatMessage, AnthropicReasoning, GeminiLLMChatMessage, ImageMimeType, LLMChatMessage, LLMFIMMessage, OpenAILLMChatMessage, RawToolParamsObj } from '../common/sendLLMMessageTypes.js';
...
import { IProjectBrainService } from './projectBrain/projectBrainService.js';
...
type SimpleLLMMessage = {
	...
	role: 'user';
	content: string;
	imageParts?: { mimeType: ImageMimeType; base64Data: string; }[];
} | {
	role: 'assistant';
	content: string;
...
// providers whose message format we construct real image content blocks for; other providers get a visible "not sent" text note instead
const VISION_CAPABLE_PROVIDERS: ProviderName[] = ['anthropic', 'openAI', 'microsoftAzure', 'gemini']
...
const prepareMessages_XML_tools = (messages: SimpleLLMMessage[], supportsAnthropicReasoning: boolean): AnthropicOrOpenAILLMMessage[] => {
	...
	for (let i = 0; i < messages.length; i += 1) {
		...
		// add user or tool to the previous user message
			...
			const cImageParts = c.role === 'user' ? c.imageParts : undefined

			if (llmChatMessages.length === 0 || llmChatMessages[llmChatMessages.length - 1].role !== 'user')
				llmChatMessages.push({
					role: 'user',
					content: c.content,
					imageParts: cImageParts,
				})
			else {
				const lastMsg = llmChatMessages[llmChatMessages.length - 1] as AnthropicLLMChatMessage & { role: 'user' }
				lastMsg.content += '\n\n' + c.content
				if (cImageParts?.length) lastMsg.imageParts = [...(lastMsg.imageParts ?? []), ...cImageParts]
			}
		}
	}
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .jcodesrules file):\n${aiInstructions}`)
	...
	for (let i = 0; i < llmMessages.length; i += 1) {
		...
		// allowed to be empty if this message (openai-style) made a tool call
		if ('tool_calls' in currMsg && currMsg.tool_calls && currMsg.tool_calls.length > 0) continue
	...
	// ================ convert staged images into real provider content blocks (last step, after all string manipulation above) ================
	// gemini is handled separately in prepareGeminiMessages, which runs on this function's output when providerName === 'gemini' -
	// leave imageParts untouched there so it can build inlineData parts itself
	if (providerName !== 'gemini') {
		for (const m of llmMessages) {
			if (m.role !== 'user') continue
			const images = m.imageParts
			if (!images?.length) continue
			delete m.imageParts

			if (!VISION_CAPABLE_PROVIDERS.includes(providerName)) {
				// selected provider isn't known to support image input - don't send a malformed/ignored request, tell the user instead
				const note = `\n\n[${images.length} image${images.length > 1 ? 's' : ''} attached but not sent - the selected model/provider does not support image input]`
				if (typeof m.content === 'string') m.content += note
				continue
			}

			if (providerName === 'anthropic') {
				const am = m as AnthropicLLMChatMessage & { role: 'user' }
				const textBlocks = typeof am.content === 'string' ? [{ type: 'text' as const, text: am.content }] : am.content
				am.content = [
					...textBlocks,
					...images.map(img => ({ type: 'image' as const, source: { type: 'base64' as const, media_type: img.mimeType, data: img.base64Data } })),
				]
			}
			else { // openAI, microsoftAzure - share the same OpenAI-compatible message shape
				const om = m as OpenAILLMChatMessage & { role: 'user' }
				const textBlocks = typeof om.content === 'string' ? [{ type: 'text' as const, text: om.content }] : om.content
				om.content = [
					...textBlocks,
					...images.map(img => ({ type: 'image_url' as const, image_url: { url: `data:${img.mimeType};base64,${img.base64Data}` } })),
				]
			}
		}
	}
...
const prepareGeminiMessages = (messages: AnthropicLLMChatMessage[]) => {
	...
	const messages2: GeminiLLMChatMessage[] = messages.map((m): GeminiLLMChatMessage | null => {
		if (m.role === 'assistant') {
			...
			const imageParts: GeminiUserPart[] = (m.imageParts ?? []).map(img => ({ inlineData: { mimeType: img.mimeType, data: img.base64Data } }))

			if (typeof m.content === 'string') {
				return { role: 'user', parts: [{ text: m.content }, ...imageParts] } satisfies GeminiLLMChatMessage
			}
			else {
				...
				return { role: 'user', parts: [...parts, ...imageParts], }
			}
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IProjectBrainService private readonly projectBrainService: IProjectBrainService,
	) {
		super()
	}

	// Read .jcodesrules files from workspace folders
	private _getJcodesRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let jcodesRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.jcodesrules')
				const { model } = this.voidModelService.getModel(uri)
				if (!model) continue
				jcodesRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return jcodesRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .jcodesrules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.voidSettingsService.state.globalSettings.aiInstructions;
		const jcodesRulesFileContent = this._getJcodesRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (jcodesRulesFileContent) ans.push(jcodesRulesFileContent)
		return ans.join('\n\n')
	}


	// system message
	private _generateChatMessagesSystemMessage = async (chatMode: ChatMode, specialToolFormat: 'openai-style' | 'anthropic-style' | 'gemini-style' | undefined, userQuery: string) => {
		...
		// only ever reads the already-built in-memory index (or null) - never blocks on a scan, so this
		// is a no-op until the user has built Project Brain at least once
		let projectBrainContext: string | null = null
		try { projectBrainContext = this.projectBrainService.getContextSummary(userQuery) }
		catch (e) { console.error('[Project Brain] failed to summarize context for chat:', e) }

		const systemMessage = chat_systemMessage({ workspaceFolders, openedURIs, directoryStr, activeURI, persistentTerminalIDs, chatMode, mcpTools, includeXMLToolDefinitions, projectBrainContext })
		...
		for (const m of chatMessages) {
			...
			if (m.role === 'assistant') {
				...
				simpleLLMMessages.push({
					role: m.role,
					content: m.content,
					imageParts: imageSelections.length ? imageSelections.map(s => ({ mimeType: s.mimeType, base64Data: s.base64Data })) : undefined,
				})
			}
		}
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const latestUserMessage = [...chatMessages].reverse().find((m): m is ChatMessage & { role: 'user' } => m.role === 'user')
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat, latestUserMessage?.displayContent ?? '')
