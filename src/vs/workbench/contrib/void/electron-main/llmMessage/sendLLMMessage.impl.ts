...
const _sendOpenAICompatibleFIM = async ({ messages: { prefix, suffix, stopTokens }, onFinalMessage, onError, settingsOfProvider, modelName: modelName_, _setAborter, providerName, overridesOfModel }: SendFIMParams_Internal) => {
	...
	openai.completions
		...
		.then(async response => {
			...
			onFinalMessage({ fullText, fullReasoning: '', anthropicReasoning: null, usage: null });
		})
		.catch(error => {
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, chatMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	...
	const options: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming = {
		...
		stream: true,
		stream_options: { include_usage: true }, // ask OpenAI-compatible servers to send a usage chunk (used for token usage stats); ignored by servers that don't support it
		...nativeToolsObj,
		...additionalOpenAIPayload
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			for await (const chunk of response) {
				// usage (usually arrives on the final chunk, once choices is empty)
				if (chunk.usage) {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				...
				onFinalMessage({ fullText: fullTextSoFar, fullReasoning: fullReasoningSoFar, anthropicReasoning: null, usage, ...toolCallObj });
			}
		})
...
const sendAnthropicChat = async ({ messages, providerName, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, overridesOfModel, modelName: modelName_, _setAborter, separateSystemMessage, chatMode, mcpTools }: SendChatParams_Internal) => {
	...
	stream.on('finalMessage', (response) => {
		...
		const usage: LLMUsage = { inputTokens: response.usage.input_tokens, outputTokens: response.usage.output_tokens }
...
const sendGeminiChat = async ({
	...
	genAI.models.generateContentStream({
		...
		.then(async (stream) => {
			...
			// Process the stream
			for await (const chunk of stream) {
				// usage (each chunk carries the cumulative total so far; the last chunk has the final counts)
				if (chunk.usageMetadata) {
					usage = { inputTokens: chunk.usageMetadata.promptTokenCount ?? 0, outputTokens: chunk.usageMetadata.candidatesTokenCount ?? 0 }
				}
