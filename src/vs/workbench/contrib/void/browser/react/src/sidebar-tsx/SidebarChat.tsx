...
import { addImageFilesToStaging, MAX_IMAGES_PER_MESSAGE, ImageAttachRejection } from '../util/imageAttachments.js';
...
const imageAttachRejectionMessage = (r: ImageAttachRejection): string =>
	r.reason === 'not_an_image' ? `${r.filename}: not an image`
		: r.reason === 'too_large' ? `${r.filename}: file too large (max 20MB)`
			: `${r.filename}: max ${MAX_IMAGES_PER_MESSAGE} images per message`
...
export const VoidChatArea: React.FC<VoidChatAreaProps> = ({
	...
	useEffect(() => {
		...
		const t = setTimeout(() => setImageWarning(null), 4000)
		return () => clearTimeout(t)
	}, [imageWarning])

	const handleImageFiles = useCallback(async (files: File[]) => {
		if (!files.length) return
		const rejections = await addImageFilesToStaging(chatThreadService, files)
		if (rejections.length) setImageWarning(rejections.map(imageAttachRejectionMessage).join(', '))
	}, [chatThreadService])
	...
	return (
		<div
			...
			className={`
				...
				border ${isDraggingOver ? 'border-void-border-1 brightness-110' : 'border-void-border-3 focus-within:border-void-border-1 hover:border-void-border-1'}
			...
			onDragOver={enableImageAttach ? (e) => { e.preventDefault(); setIsDraggingOver(true) } : undefined}
			onDragLeave={enableImageAttach ? (e) => { e.preventDefault(); setIsDraggingOver(false) } : undefined}
			onDrop={enableImageAttach ? (e) => {
				e.preventDefault()
		...
		>
			...
			{imageWarning && <WarningBox text={imageWarning} className='px-0.5' />}
			...
			<div className='flex flex-row justify-between items-end gap-1'>
				...
				<div className="flex items-center gap-2">
					...
					{enableImageAttach && !isStreaming && (
						<ButtonAttachImage onFilesSelected={handleImageFiles} />
					)}
...
export const ButtonSubmit = ({ className, disabled, ...props }: ButtonProps & Required<Pick<ButtonProps, 'disabled'>>) => {
	...
	return <button
		type='button'
		className={`rounded-full flex-shrink-0 flex-grow-0 flex items-center justify-center text-vscode-button-fg
...
const ButtonAttachImage = ({ className, onFilesSelected, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { onFilesSelected: (files: File[]) => void }) => {
	const fileInputRef = useRef<HTMLInputElement | null>(null)
	return <>
		<button
			type='button'
			className={`rounded-full flex-shrink-0 flex-grow-0 flex items-center justify-center text-void-fg-3 hover:text-void-fg-1 cursor-pointer
				${className}
			`}
			data-tooltip-id='void-tooltip'
			...
			{allSelections.map((selection, i) => {
				...
				>
					...
					<span className="truncate overflow-hidden text-ellipsis"
						data-tooltip-id='void-tooltip'
						data-tooltip-content={selection.type === 'Image' ? selection.filename : getRelative(selection.uri, accessor)}
						data-tooltip-place='top'
						...
						<div
							...
							onClick={() => {
								...
								if (isThisSelectionProspective) { // add prospective selection to selections
									...
									// no-op - synthetic URI, nothing to open
								}
							}}
						>
							{selection.type === 'Image'
								? <img src={`data:${selection.mimeType};base64,${selection.base64Data}`} className="w-2.5 h-2.5 object-cover rounded-[1px]" />
								: <SelectionIcon size={10} />
							}

							{ // file name and range
								selection.type === 'Image' ? selection.filename
...
const ToolHeaderWrapper = ({
	...
	return (<div className=''>
		<div className={`w-full rounded-sm overflow-hidden ${className}`}>
			{/* header */}
			<div className={`select-none flex items-center min-h-[22px] -mx-1 px-1 rounded-sm ${isClickable ? 'hover:bg-void-bg-2/60' : ''} transition-colors duration-100`}>
			...
			{/* children */}
			{<div
				className={`overflow-hidden transition-all duration-200 ease-in-out ${isExpanded ? 'opacity-100 py-1 mt-0.5' : 'max-h-0 opacity-0'}
					text-void-fg-4 overflow-x-auto
					border-l border-void-border-3 ml-1.5 pl-2.5
				  `}
			>
				{children}
			</div>}
...
const AssistantMessageComponent = ({ chatMessage, isCheckpointGhost, isCommitted, messageIdx }: { chatMessage: ChatMessage & { role: 'assistant' }, isCheckpointGhost: boolean, messageIdx: number, isCommitted: boolean }) => {
	...
	// defensively hide placeholder text some models echo back when a turn is just a tool call with no prose
	const isPlaceholderContent = chatMessage.displayContent?.trim() === '(empty message)'
	const isEmpty = (!chatMessage.displayContent || isPlaceholderContent) && !chatMessage.reasoning
	...
	return <>
		...
		{/* assistant message */}
		{chatMessage.displayContent && !isPlaceholderContent &&
			<div className={`${isCheckpointGhost ? 'opacity-50' : ''}`}>
				<ProseWrapper>
					<ChatMarkdownRender
...
const CommandBarInChat = () => {
	...
	// hide the whole bar when there are no changed files
	...
	const numFilesChangedStr = `${sortedCommandBarURIs.length} file${numFilesChanged === 1 ? '' : 's'} with changes`
	...
	const fileDetailsButton = (
		<button
			className='flex items-center gap-1 rounded cursor-pointer hover:brightness-125 transition-all duration-200'
