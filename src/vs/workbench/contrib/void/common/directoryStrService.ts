...
// exported for reuse by other services that need the same directory walk exclusions (eg projectBrainScanner.ts)
...
// exported for reuse by other services that need one-more-level-deep resolution (eg projectBrainScanner.ts)
export const resolveChildren = async (children: undefined | IFileStat[], fileService: IFileService): Promise<IFileStat[]> => {
