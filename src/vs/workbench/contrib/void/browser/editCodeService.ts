...
import { estimateTokenCount } from '../common/tokenUsageTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	private _instantlyApplySRBlocks(uri: URI, blocksStr: string) {
		...
		// real efficiency stat: what we actually sent (the S/R blocks) vs. what the alternative "rewrite the
		// whole file" mechanism (_initializeWriteoverStream) would have cost for this same edit - see tokenUsageService.ts
		const diffTokens = blocks.reduce((sum, b) => sum + estimateTokenCount(b.orig) + estimateTokenCount(b.final), 0)
		this._tokenUsageService.recordEditSavings(estimateTokenCount(modelStr), diffTokens)
