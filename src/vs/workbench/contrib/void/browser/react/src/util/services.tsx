...
import { IProjectBrainService, ProjectBrainState } from '../../../projectBrain/projectBrainService.js'
import { ProjectBrainNavigationRequest } from '../../../../common/projectBrain/projectBrainTypes.js'
import { IGitHubAccountService, GitHubAccountsState } from '../../../voidGitHubService.js'
...
const tokenUsageChangeListeners: Set<() => void> = new Set()

let projectBrainState: ProjectBrainState
const projectBrainStateListeners: Set<(s: ProjectBrainState) => void> = new Set()
const projectBrainNavigationListeners: Set<(nav: ProjectBrainNavigationRequest) => void> = new Set()

let githubAccountsState: GitHubAccountsState
const githubAccountsStateListeners: Set<(s: GitHubAccountsState) => void> = new Set()
...
export const _registerServices = (accessor: ServicesAccessor) => {
	...
	const stateServices = {
		...
		projectBrainService: accessor.get(IProjectBrainService),
		githubAccountService: accessor.get(IGitHubAccountService),
	}

	const { settingsStateService, chatThreadsStateService, refreshModelService, themeService, editCodeService, voidCommandBarService, modelService, mcpService, tokenUsageService, projectBrainService, githubAccountService } = stateServices
	...
	disposables.push(
		tokenUsageService.onDidChangeUsage(() => {
			tokenUsageChangeListeners.forEach(l => l())
		})
	)

	projectBrainState = projectBrainService.state
	disposables.push(
		projectBrainService.onDidChangeState(() => {
			projectBrainState = projectBrainService.state
			projectBrainStateListeners.forEach(l => l(projectBrainState))
		})
	)
	disposables.push(
		projectBrainService.onDidRequestNavigate((nav) => {
			projectBrainNavigationListeners.forEach(l => l(nav))
		})
	)

	githubAccountsState = githubAccountService.state
	disposables.push(
		githubAccountService.onDidChangeState(() => {
			githubAccountsState = githubAccountService.state
			githubAccountsStateListeners.forEach(l => l(githubAccountsState))
		})
	)
...
const getReactAccessor = (accessor: ServicesAccessor) => {
	const reactAccessor = {
		...
		IProjectBrainService: accessor.get(IProjectBrainService),
		IGitHubAccountService: accessor.get(IGitHubAccountService),
...
// fires whenever recorded token usage changes (eg a chat message finishes streaming)
export const useTokenUsageChangeListener = (listener: () => void) => {
	useEffect(() => {
		tokenUsageChangeListeners.add(listener)
		return () => { tokenUsageChangeListeners.delete(listener) }
	}, [listener])
}


export const useProjectBrainState = () => {
	const [s, ss] = useState(projectBrainState)
	useEffect(() => {
		ss(projectBrainState)
		projectBrainStateListeners.add(ss)
		return () => { projectBrainStateListeners.delete(ss) }
	}, [ss])
	return s
}

// fires whenever a command (eg "Show Architecture") asks an open/opening dashboard to jump to a specific tab
export const useProjectBrainNavigationListener = (listener: (nav: ProjectBrainNavigationRequest) => void) => {
	useEffect(() => {
		projectBrainNavigationListeners.add(listener)
		return () => { projectBrainNavigationListeners.delete(listener) }
	}, [listener])
}

export const useGitHubAccountsState = () => {
	const [s, ss] = useState(githubAccountsState)
	useEffect(() => {
		ss(githubAccountsState)
		githubAccountsStateListeners.add(ss)
		return () => { githubAccountsStateListeners.delete(ss) }
	}, [ss])
	return s
}
