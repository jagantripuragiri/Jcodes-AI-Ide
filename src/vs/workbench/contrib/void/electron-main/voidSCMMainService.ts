...
import { IVoidSCMService, GitActivityCommit } from '../common/voidSCMTypes.js'
...
// unit/record separators (not printable, won't collide with real commit text) used to delimit
// `git log` output into machine-parseable commits/fields for gitRecentActivity
...
export class VoidSCMService implements IVoidSCMService {
	...
	async gitRecentActivity(path: string, limit: number = 20): Promise<GitActivityCommit[]> {
		const raw = await git(
			`git log --name-only --no-merges -n ${limit} --pretty=format:"${RECORD_SEP}%H${UNIT_SEP}%s${UNIT_SEP}%ad" --date=iso-strict`,
			path
		)

		const commits: GitActivityCommit[] = []
		const blocks = raw.split(RECORD_SEP).map((b) => b.trim()).filter((b) => b.length > 0)

		for (const block of blocks) {
			const newlineIdx = block.indexOf('\n')
			const header = newlineIdx === -1 ? block : block.slice(0, newlineIdx)
			const rest = newlineIdx === -1 ? '' : block.slice(newlineIdx + 1)
			const [hash, subject, date] = header.split(UNIT_SEP)
			if (!hash) continue
			const filesChanged = rest.split('\n').map((l) => l.trim()).filter((l) => l.length > 0).slice(0, MAX_FILES_PER_COMMIT)
			commits.push({ hash, subject: subject ?? '', date: date ?? '', filesChanged })
		}
